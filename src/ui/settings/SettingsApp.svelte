<script lang="ts">
  ...
  onMount(() => {
    ...
    container.createEl("h3", { text: "Gamification & UX" });

    new Setting(container)
      .setName("Enable Audio")
      .setDesc("Play sound effects during flashcard review.")
      .addToggle((toggle) =>
        toggle
          .setValue(plugin.settings.enableAudio)
          .onChange(async (value) => {
            plugin.settings.enableAudio = value;
            await plugin.saveSettings();
          }),
      );
    ...
    new Setting(container)
      ...
      .addToggle((toggle) =>
        toggle
          .setValue(plugin.settings.enableAnimations)
          .onChange(async (value) => {
            plugin.settings.enableAnimations = value;
            await plugin.saveSettings();
          }),
      );
